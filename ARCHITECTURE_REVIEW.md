...
| Merge safety depended on a local convention. | `.github/workflows/ci.yml` now runs the full unit suite, focused overhaul and UX suites, production dependency audit, Prisma validation, pricing red-team, TypeScript, production build and all deterministic Playwright dealer/offline/golden paths. | Repository branch protection must still mark `ship-gates` required in GitHub settings; workflow YAML cannot enforce that external setting. |
...
- Live provider and DB-backed E2E in CI: CI has no disposable seeded Postgres service. The deterministic route-intercepted real-UI proof is reproducible; add a staging smoke test when an isolated database is available.
...
- Ship gates: CI workflow now encodes production audit, Prisma validation, full/focused tests, red-team, typecheck, build and all browser proofs; `verify:prod` remains the explicit post-deploy release check.
