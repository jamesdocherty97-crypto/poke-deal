...
5. **Add a disposable seeded Postgres browser lane.** Current E2E tests use the real UI with deterministic route fixtures. That is fast and reproducible, but it cannot expose migration/constraint/transaction regressions that only a real database shows.
