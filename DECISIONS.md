...
- **Decision:** Pin Next.js to the current security-backport release `15.5.21` while retaining React/React DOM `18.3.1`, scope-override Next's bundled PostCSS to an audit-clean `8.5.x` patch (currently `8.5.25`), and resolve `@vercel/blob`'s compatible Undici range to the audit-clean `6.28.0` patch in the authoritative lockfile. Convert dynamic route-handler params to Next 15's asynchronous contract. Do not combine this security move with a React 19 migration.
- **Why:** The locked Next 14 release reports high-severity production advisories and has no audit-clean patch on that major line. The Next 15.5 security-backport line accepts React 18.3.1 and avoids the much larger Next 16/React 19 change during an already broad reliability pass. The exact PostCSS override closes its transitive advisory without admitting a future major.
...
- **Guards:** Dependency audit with zero findings; full unit/overhaul/UX/pricing suites; plain TypeScript plus Next production build; all Playwright dealer/offline/golden paths; representative dynamic-route and access/cron smoke checks. Authentication details are superseded by A12.
...
- **Guards:** Pure index-definition tests plus a cleanly skippable physical PostgreSQL assertion, deep-health visibility, exact-path middleware review, production smoke checks for `/` and private APIs remaining denied while `/robots.txt` returns only the disallow policy, and the read-only `npm run drift:checked` audit for provider-versus-qualified-checked evidence.

## 2026-08-01 — Trusted-browser access

### A12. Trust each browser once, then keep normal app launches password-free

- **Decision:** Replace the HTTP Basic password gate with a private `/access#TOKEN` enrollment link. Client code removes the fragment before sending the token in a bounded, same-origin JSON request; the server returns a uniquely identified, signed, host-only, `Secure`, `HttpOnly`, `SameSite=Strict` session cookie. Sessions last 180 days and renew only during the final 30 days of normal use. Production and hosted previews require distinct high-entropy `APP_ACCESS_TOKEN` and `APP_SESSION_SECRET` values and fail closed otherwise. `APP_PUBLIC_ACCESS` works only on loopback and is ignored in every Vercel environment. Cron routes remain bearer-only. Public middleware exceptions are exact and limited to the crawler policy, non-sensitive PWA install assets, enrollment itself, the independently verified eBay deletion callback, and the signed/state-bound eBay OAuth return.
- **Why:** The owner expects the installed app to open directly, while the workspace still holds sensitive stock, costs, sales, and provider configuration. Browser trust removes the recurring prompt without making the deployment public. Strict cookies reduce cross-site request risk; eBay OAuth cannot carry that cookie on a cross-site return, so only its exact callback is exempt and its short-lived signed browser-bound state remains the authorization check.
